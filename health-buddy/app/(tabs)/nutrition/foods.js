...
export default function Foods() {
  ...
  const kcal = useMemo(()=> (4*Number(p||0) + 4*Number(c||0) + 9*Number(f||0)).toFixed(1), [p,c,f]);
  ...
  const add = useCallback(async () => {
    try{
      const body = { name_vi:name.trim(), portion_g:Number(portion||0), protein_g:Number(p||0), carbs_g:Number(c||0), fat_g:Number(f||0), fiber_g:Number(fib||0), kcal:Number(kcal) };
      if (!body.name_vi || !body.portion_g) return Alert.alert('Thiếu thông tin', 'Tên món & khẩu phần bắt buộc');
    ...
    }catch(_e){ Alert.alert('Lỗi', 'Không thể thêm'); }
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <View style={{ padding:16 }}>
          ...
          <TextInput value={q} onChangeText={setQ} placeholder="Tìm kiếm…" placeholderTextColor="#9ca3af"
          ...
          <View style={{ backgroundColor:'#fff', borderWidth:1, borderColor:C.b, borderRadius:12, padding:12, marginTop:12 }}>
            ...
            <TextInput value={name} onChangeText={setName} placeholder="Tên món" placeholderTextColor="#9ca3af"
              style={{ marginTop:8, backgroundColor:'#fff', borderWidth:1, borderColor:C.b, borderRadius:8, padding:8 }} />
            <TextInput value={portion} onChangeText={setPortion} keyboardType="numeric" placeholder="Khẩu phần (g)"
              style={{ marginTop:8, backgroundColor:'#fff', borderWidth:1, borderColor:C.b, borderRadius:8, padding:8 }} />
